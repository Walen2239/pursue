...
$jobListings = [];
$selectedCategory = ''; // This will store the category passed in the URL
...
if (isset($_GET['category']) && !empty($_GET['category'])) {
    ...
    // If no category is specified, redirect to the main job listings page or show an error
    header("Location: appindex.php"); // Redirect to appindex.php if no category
    exit();
}

$sql = "SELECT listingsID, job_title, company_name, description, location, salary, job_type, date_posted, category
        FROM listings
        WHERE approval = 1 AND category = '" . $selectedCategory . "'"; // Filter by the selected category
...
<head>
    ...
    <title>Job Portal - <?php echo htmlspecialchars($selectedCategory); ?> Opportunities</title>
    ...
    <style>
        ...
        .category-box.active {
            border: 2px solid #007bff; /* Example active style */
            ...
            <aside class="col-md-3 col-lg-2 sidebar p-4 bg-dark">
                ...
                <nav class="nav flex-column">
                    <?php if (isset($_SESSION['appemail'])): ?>
                        ...
                        <a class="nav-link text-white" href="appindex.php">All Category</a> <a class="nav-link text-white" href="applied_listing.php">Applied Listings</a>
                        <a href="../includes/logout.php" class="nav-link text-red" style="margin-top:2rem">Logout</a>
                    <?php elseif (isset($_SESSION['recemail'])): ?>
                        <a class="nav-link text-white" href="#">About Us</a>
                        <a class="nav-link text-white" href="appindex.php">All Category</a> <a href="../recruitor/recindex.php" style="margin-top:2rem" class="nav-link text-white">New Listing</a>
                    ...
                    <?php else: ?>
                        ...
                        <a class="nav-link text-white" href="appindex.php">All Category</a> <?php endif; ?>
                </nav>
            </aside>
            <main class="col-md-9 col-lg-10 p-4 main-content">
                <div class="row justify-content-center g-4">
                    ...
                    foreach ($categories as $cat) {
                        $isActive = ($selectedCategory === $cat) ? 'active' : '';
                        echo '<div class="col-md-2">';
                        echo '<a href="category_details.php?category=' . urlencode($cat) . '" class="category-box text-decoration-none ' . $isActive . '">';
                ...
                <div class="row justify-content-center g-4 mt-2">
                    <?php if (!empty($jobListings)): ?>
                        ...
                        <?php endforeach; ?>
                    <?php else: ?>
                        <p class="text-center w-100">No approved job listings available for the category "<?php echo htmlspecialchars($selectedCategory); ?>" at the moment.</p>
                    <?php endif; ?>
                </div>
            </main>
        </div>
