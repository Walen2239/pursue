...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($stmt) {
        ...
        header("Location: ../recruitor/recsignup.php");
        exit();
    }

    // Insert recruiter
    $sql = "INSERT INTO recruiters (recFName, recLName, recEmail, recPassword) VALUES (?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);

    if ($stmt) {
        $stmt->bind_param("ssss", $recfname, $reclname, $recemail, $recpwd);
